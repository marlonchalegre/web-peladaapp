            ...
            <ListItem key={`${side}-${tp.player_id}`} divider sx={{ gap: 1 }}>
              {side === "home" ? (
                <>
                  ...
                  {openAction?.key === actionKey &&
                    openAction?.type === "assist" && (
                      <Stack direction="row" spacing={0.5} alignItems="center">
                        ...
                        <IconButton
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            if (finished) return;
                            await recordEvent(m.id, tp.player_id, "assist");
                          }}
                        >
                      ...
                      <Stack direction="row" spacing={0.5} alignItems="center">
                        ...
                        <IconButton
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            if (finished) return;
                            await recordEvent(m.id, tp.player_id, "goal");
                            await adjustScore(m, "home", 1);
                          }}
                        >
                          <AddIcon fontSize="small" />
                      ...
                      <Stack direction="row" spacing={0.5} alignItems="center">
                        ...
                        <IconButton
                          ...
                          onClick={async (e) => {
                            e.stopPropagation();
                            if (finished) return;
                            await recordEvent(m.id, tp.player_id, "own_goal");
                            await adjustScore(m, "away", 1);
                          }}
                        >
                          <AddIcon fontSize="small" />
                  ...
                  <ListItemText
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (finished) return;
                      setSelectMenu({ teamId, forPlayerId: tp.player_id });
                    }}
                    primary={name}
                  />
                </>
                    ...
                    <Stack>
                      {benchPlayers.map((p) => (
                        ...
                        >
                          <ListItemText
                            primary={
                              userIdToName[p.user_id] ?? `Player #${p.id}`
                            }
                          />
                        </IconButton>
                      ))}
                    ...
                    {benchPlayers.map((p) => (
                      <IconButton
                        ...
                        onClick={async (e) => {
                          e.stopPropagation();
                          if (finished) return;
                          await assignPlayerToTeam(teamId, p.id);
                        }}
                      >
                        <ListItemText
                          primary={userIdToName[p.user_id] ?? `Player #${p.id}`}
                        />
                      </IconButton>
                    ))}
                    <IconButton
  ...
  return (
    <Paper sx={{ p: 2, m: 1 }}>
      <Stack direction="row" spacing={3}>
        <div style={{ flex: 1 }}>
          {renderTeamList(m.home_team_id, homePlayers, "home")}
        </div>
        <div style={{ flex: 1 }}>
          {renderTeamList(m.away_team_id, awayPlayers, "away")}
        </div>
      </Stack>
    </Paper>
  );
