  ...
  return (
    <Paper variant="outlined" sx={{ mb: 2, overflow: 'hidden' }}>
      <Box sx={{ bgcolor: 'action.hover', p: 1.5, borderBottom: 1, borderColor: 'divider' }}>
         <Typography variant="subtitle1" fontWeight="bold">{t('peladas.panel.standings.title')}</Typography>
      </Box>
      <TableContainer>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 'bold' }}>{t('common.team')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold' }}>{t('common.wins_short')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold' }}>{t('common.draws_short')}</TableCell>
              <TableCell align="center" sx={{ fontWeight: 'bold' }}>{t('common.losses_short')}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {standings.map((row, index) => (
              ...
              >
                <TableCell>{row.name || t('peladas.matches.team_fallback', { id: row.teamId })}</TableCell>
