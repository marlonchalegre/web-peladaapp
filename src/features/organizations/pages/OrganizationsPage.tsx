...
export default function OrganizationsPage() {
  ...
  const [addingPlayersOrgId, setAddingPlayersOrgId] = useState<number | null>(
    null,
  );
  const [managingAdminsOrg, setManagingAdminsOrg] =
  ...
  useEffect(() => {
    setLoading(true);
    api
      .getPaginated<Organization[]>("/api/organizations", {
        page,
        per_page: perPage,
      })
      .then((data) => {
      ...
      .catch((error: unknown) => {
        const message =
          error instanceof Error
            ? error.message
            : t("organizations.list.error.load_failed");
        setError(message);
      })
        ...
        onAddPlayers={async (o) => {
          try {
            ...
            const message =
              error instanceof Error
                ? error.message
                : t("organizations.error.load_users_failed");
            setError(message);
          }
        }}
        onManageAdmins={async (o) => {
          try {
            ...
            const message =
              error instanceof Error
                ? error.message
                : t("organizations.error.load_admins_failed");
            setError(message);
          } finally {
          ...
          await Promise.all(
            ids.map((uid) =>
              endpoints.createPlayer({
                organization_id: addingPlayersOrgId,
                user_id: uid,
              }),
            ),
          );
      ...
      {managingAdminsOrg && (
        <ManageAdminsDialog
          ...
          onAddAdmin={async (userId: number) => {
            const newAdmin = await endpoints.addOrganizationAdmin(
              managingAdminsOrg.id,
              userId,
            );
            setAdmins((prev) => [...prev, newAdmin]);
          }}
          onRemoveAdmin={async (userId: number) => {
            await endpoints.removeOrganizationAdmin(
              managingAdminsOrg.id,
              userId,
            );
