...
import { createApi, type Organization, type User, type Player, type OrganizationAdmin } from '../../../shared/api/endpoints'
...
export default function OrganizationManagementPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('organizations.error.load_failed')
      setError(message)
    } finally {
      setLoading(false)
    }
  ...
  const handleRemovePlayer = async (playerId: number) => {
    if (!window.confirm(t('organizations.management.remove_member_confirm'))) return
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('organizations.error.delete_failed')
      setError(message)
    } finally {
  ...
  const handleAddAdmin = async () => {
    ...
    setActionLoading(true)
    try {
      await endpoints.addOrganizationAdmin(orgId, selectedAdminUserId as number)
  ...
  const handleAddPlayers = async () => {
    ...
    try {
      ...
      await Promise.all(ids.map((uid) => endpoints.createPlayer({ organization_id: orgId, user_id: uid })))
      setIsAddPlayersOpen(false)
      setSelectedUserIds(new Set())
      await fetchData()
    } catch (err) {
      const message = err instanceof Error ? err.message : t('organizations.error.add_players_failed', { defaultValue: 'Error adding players' })
      setError(message)
    } finally {
      setActionLoading(false)
    }
  ...
  if (!org) return <Container sx={{ mt: 4 }}><Alert severity="error">{error || t('organizations.error.load_failed')}</Alert></Container>
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Stack spacing={4}>
        ...
        <Paper variant="outlined" sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="h5">{t('organizations.management.sections.members')}</Typography>
            <Button 
              variant="contained" 
          ...
          <List>
            {players.length === 0 ? (
              <Typography color="text.secondary">{t('organizations.list.empty')}</Typography>
            ) : (
              players.map((player) => {
        ...
        {/* Admins Section */}
        <Paper variant="outlined" sx={{ p: 3 }}>
          <Typography variant="h5" gutterBottom>{t('organizations.management.sections.admins')}</Typography>
          <Divider sx={{ mb: 2 }} />
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <TextField
                ...
                onChange={(e) => setSelectedAdminUserId(e.target.value === '' ? '' : Number(e.target.value))}
                disabled={actionLoading || playersNotAdmins.length === 0}
                label={playersNotAdmins.length === 0 ? t('organizations.dialog.manage_admins.all_users_are_admins') : t('organizations.dialog.manage_admins.select_user_label')}
              >
      ...
      >
        ...
        <DialogActions>
          ...
          >
            {actionLoading ? <CircularProgress size={24} /> : t('common.actions')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <AddPlayersDialog
        ...
        onSelectAll={() => setSelectedUserIds(new Set(usersNotPlayers.map(u => u.id)))}
        ...
        onAddAll={async () => {
          ...
          setActionLoading(true)
          try {
            await Promise.all(allIds.map((uid) => endpoints.createPlayer({ organization_id: orgId, user_id: uid })))
            setIsAddPlayersOpen(false)
            await fetchData()
          } catch (err) {
            setError(err instanceof Error ? err.message : String(err))
