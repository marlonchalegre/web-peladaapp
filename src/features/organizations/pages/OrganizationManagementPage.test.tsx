...
describe('OrganizationManagementPage', () => {
  ...
  const mockAdmins = [
    { id: 201, organization_id: 1, user_id: 1, user_name: 'User One', user_email: 'user1@example.com' }
  ]
  const mockUsers = [
  ...
  beforeEach(() => {
    ...
    vi.mocked(api.get).mockImplementation((url) => {
      ...
      if (url === '/api/organizations/1/players') return Promise.resolve(mockPlayers)
      if (url === '/api/organizations/1/admins') return Promise.resolve(mockAdmins)
    ...
    return render(
      <AuthProvider>
        <MemoryRouter initialEntries={['/organizations/1/management']}>
          <Routes>
            <Route path="/organizations/:id/management" element={<OrganizationManagementPage />} />
          </Routes>
        </MemoryRouter>
      </AuthProvider>
    )
  ...
  it('renders organization details and sections after loading', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('organizations.management.title')).toBeInTheDocument()
    })

    expect(screen.getByText('organizations.management.sections.members')).toBeInTheDocument()
    expect(screen.getByText('organizations.management.sections.admins')).toBeInTheDocument()
    expect(screen.getByText('organizations.management.sections.danger_zone')).toBeInTheDocument()
  ...
  it('handles removing a member', async () => {
    ...
    const membersHeading = screen.getByText('organizations.management.sections.members')
    // Paper is the closest div usually in MUI
    const membersSection = membersHeading.closest('div.MuiPaper-root') as HTMLElement
  ...
  it('handles adding a new admin', async () => {
    vi.mocked(api.post).mockResolvedValue({ id: 202, organization_id: 1, user_id: 2 })
    renderComponent()
    ...
    const selectLabel = screen.getByLabelText('organizations.dialog.manage_admins.select_user_label')
    ...
    const adminsHeading = screen.getByText('organizations.management.sections.admins')
    const adminsSection = adminsHeading.closest('div.MuiPaper-root') as HTMLElement
    const addBtn = within(adminsSection!).getByText('common.add')
    
    fireEvent.click(addBtn)

    expect(api.post).toHaveBeenCalledWith('/api/organizations/1/admins', { user_id: 2 })
    await waitFor(() => {
  ...
  it('handles removing an admin', async () => {
    const twoAdmins = [
      ...mockAdmins,
      { id: 202, organization_id: 1, user_id: 2, user_name: 'User Two', user_email: 'user2@example.com' }
    ]
    vi.mocked(api.get).mockImplementation((url) => {
        if (url === '/api/organizations/1/admins') return Promise.resolve(twoAdmins)
        if (url === '/api/organizations/1') return Promise.resolve(mockOrg)
        if (url === '/api/organizations/1/players') return Promise.resolve(mockPlayers)
        if (url === '/api/users') return Promise.resolve(mockUsers)
        return Promise.reject(new Error(`Unhandled GET: ${url}`))
    ...
    const userTwoAdminItem = within(adminList).getByText('User Two').closest('li')
  ...
  it('prevents removing the last admin', async () => {
    ...
    expect(deleteBtn).toHaveAttribute('title', 'organizations.dialog.manage_admins.cannot_remove_last_admin_tooltip')
  ...
  it('handles adding players flow', async () => {
    ...
    expect(screen.getByText('organizations.dialog.add_players.title')).toBeInTheDocument()
    
    const userThreeCheckbox = screen.getByText('User Three').closest('li')?.querySelector('input[type="checkbox"]')
    fireEvent.click(userThreeCheckbox!)

    const addSelectedBtn = screen.getByText('organizations.dialog.add_players.add_selected')
    fireEvent.click(addSelectedBtn)

    expect(api.post).toHaveBeenCalledWith('/api/players', { organization_id: 1, user_id: 3 })
    await waitFor(() => {
       expect(api.get).toHaveBeenCalledWith('/api/organizations/1/players')
    })
  })
  ...
  it('handles organization deletion with confirmation', async () => {
    ...
    expect(screen.getByText('organizations.management.delete_confirm_title')).toBeInTheDocument()
    ...
    const confirmBtn = within(dialogActions as HTMLElement).getAllByRole('button')[1]
