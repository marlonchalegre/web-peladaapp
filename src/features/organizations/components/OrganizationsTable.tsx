          ...
          <TableBody>
            {orgs.map((o) => (
              <TableRow key={`org-${o.id}`}>
                ...
                <TableCell>
                  <IconButton aria-label={t('organizations.table.aria.add_players', { name: o.name })} onClick={() => onAddPlayers(o)} size="small" sx={{ mr: 1 }}>
                    <PersonAddIcon fontSize="small" />
                  </IconButton>
                  {onManageAdmins && (
                    <IconButton aria-label={t('organizations.table.aria.manage_admins', { name: o.name })} onClick={() => onManageAdmins(o)} size="small" sx={{ mr: 1 }} color="primary">
                      <AdminPanelSettingsIcon fontSize="small" />
                    </IconButton>
                  )}
                  <IconButton aria-label={t('organizations.table.aria.delete', { name: o.name })} onClick={() => onDelete(o)} size="small" color="error">
